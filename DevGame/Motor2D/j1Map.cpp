		...
		for (layer_pointer; layer_pointer != nullptr; layer_pointer = layer_pointer->next)
		{
			for (uint x = 0; x < data.width; x++)
			{
				for (uint y = 0; y < data.height; y++)
				{
					...
					if (layer_pointer->data->name == "Navigation") {
						...
						App->render->Blit(tileset_pointer->data->texture, coordinates.x, coordinates.y, &rect, 1.0f, 50.0f);
				}
			}
		}
	}
...
iPoint j1Map::WorldToMap(int x, int y) const
{
	...
	case MAPTYPE_ORTHOGONAL:
		...
		break;
	case MAPTYPE_ISOMETRIC:
		ret.x = (x / data.tile_width * 0.5) + y;
		ret.y = (y / data.tile_height* 0.5) - x;
		break;
	}
	return ret;
}
...
{
	BROFILER_CATEGORY("Load", Profiler::Color::MediumSpringGreen)
	bool ret = true;
...
bool j1Map::LoadMap()
{
	BROFILER_CATEGORY("LoadMap", Profiler::Color::MediumSpringGreen)
	bool ret = true;
...
{
	BROFILER_CATEGORY("LoadTiledDetails", Profiler::Color::MediumSpringGreen)
	bool ret = true;
...
{
	BROFILER_CATEGORY("LoadTilesetImage", Profiler::Color::MediumSpringGreen)
	bool ret = true;
...
{
	BROFILER_CATEGORY("LoadLayer", Profiler::Color::MediumSpringGreen)
	bool ret = true;
